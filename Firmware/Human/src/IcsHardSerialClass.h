...
class IcsHardSerialClass : public IcsBaseClass
{
  ...
  public:
	...
	IcsHardSerialClass(HardwareSerial* icsSerial,byte enpin,long baudrate,int timeout, int rxpin, int txpin);    
  ...
  protected: 
	...
	int timeOut;               ///<通信のタイムアウト(ms)を格納しておく変数
       ...
       public:
        ...
        bool begin(HardwareSerial *serial, int enpin, long baudrate,
                   int timeout);
	...
	**/
	inline void enHigh(){digitalWrite(enPin, LOW);}
	/**
	*	@brief enPinに割り当てられているピンをLにする
	**/
	inline void enLow(){digitalWrite(enPin, HIGH);}
